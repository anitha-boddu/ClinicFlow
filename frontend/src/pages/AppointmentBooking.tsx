...
import { User, Calendar, CreditCard, CheckCircle, Download, Clock } from 'lucide-react';
...
export default function AppointmentBooking() {
  ...
  useEffect(() => {
    if (appointmentDate && selectedDoctor) {
      fetch(`${apiUrl}/api/medical-appointments/booked-tokens/${selectedDoctor._id}/${appointmentDate}`)
        .then(res => res.json())
  ...
  const normalizedDoctors = doctors.map(doc => ({ ...doc, address: normalizeHospitalName(doc.address) }));
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      {/* Removed Live Token label and map/location features */}
                    </div>
                  </div>
                );
              })}
