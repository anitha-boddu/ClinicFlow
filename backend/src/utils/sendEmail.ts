...
export const sendEmail = async (email: string, otp: string, subjectLine?: string, titleText?: string) => {
  try {
    const finalSubject = subjectLine || "ClinicFlow Email Verification OTP";
    const finalTitleText = titleText || "Verify Your Email Address";
    const mailOptions = {
      from: `"ClinicFlow" <${process.env.SENDER_EMAIL || "main@clinicflow.in"}>`,
      to: email,
      subject: finalSubject,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
          <div style="background-color: #0066CC; padding: 20px; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">ClinicFlow Registration</h1>
          </div>
          <div style="padding: 30px; background-color: #ffffff;">
            ...
            <div style="background-color: #f4f4f5; border-radius: 6px; padding: 15px; margin: 25px 0; text-align: center;">
              <span style="font-size: 32px; font-weight: bold; letter-spacing: 4px; color: #0066CC;">${otp}</span>
            </div>
          ...
          <div style="background-color: #f9fafb; padding: 15px; text-align: center; border-top: 1px solid #eeeeee;">
            <p style="color: #888888; font-size: 12px; margin: 0;">© ${new Date().getFullYear()} ClinicFlow. All rights reserved.</p>
          </div>
        </div>
      `,
    };
